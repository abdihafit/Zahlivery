...
class RiderChatsScreen extends StatefulWidget {
  const RiderChatsScreen({super.key, required this.rider});

  final AppUser rider;

  @override
  State<RiderChatsScreen> createState() => _RiderChatsScreenState();
}

class _RiderChatsScreenState extends State<RiderChatsScreen> {
      ...
      body: StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
        ...
        builder: (context, snapshot) {
          ...
          if (docs.isEmpty) {
            return const Center(
              child: Text(
                'No chats yet. Customers will appear here when they contact you.',
              ),
            );
          }
                            ...
                            MaterialPageRoute(
                              builder: (_) => CustomerChatScreen(
                                ...
                                peerUser: AppUser(
                                  ...
                                  phone: '',
                                  businessName:
                                      peerRole == UserRole.hotel ? peerName : null,
                                ),
                                chatId: doc.id,
                                chatTitle:
                                    peerName.isEmpty ? peerRole.label : peerName,
                              ),
                            ),
                          );
                        },
